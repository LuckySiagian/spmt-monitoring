      ...
      <div style={{ background: 'var(--bg-main)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-header)', flexShrink: 0, flexWrap: 'wrap', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--text-muted)', fontWeight: 600 }}>Complete history · {events.length} events</span>
          </div>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {FILTERS.map(f => (
              <button key={f} onClick={() => { setFilter(f); setPage(1) }}
                style={{ background: filter === f ? 'rgba(79,70,229,0.12)' : 'rgba(99,102,241,0.05)', border: filter === f ? '1px solid rgba(79,70,229,0.4)' : '1px solid rgba(99,102,241,0.15)', color: filter === f ? '#4f46e5' : 'var(--text-muted)', fontSize: 13, fontWeight: 800, padding: '6px 14px', borderRadius: 4, cursor: 'pointer', letterSpacing: '0.06em' }}>{f}</button>
            ))}
          </div>
        </div>
        {/* Table */}
                      ...
                      <tr key={ev.id} style={{ borderBottom: '1px solid rgba(99,102,241,0.06)', background: i % 2 === 0 ? 'transparent' : 'rgba(99,102,241,0.02)' }}
                        ...
                        <td style={{ padding: '16px 18px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <span style={{ color: SC[ev.old_status] || 'var(--text-muted)', fontSize: 14, opacity: 0.6, fontWeight: 600 }}>{ev.old_status || '—'}</span>
                            <ArrowRight size={14} style={{ color: 'var(--text-muted)' }} />
                            <span style={{ color: SC[ev.new_status] || 'var(--text-muted)', fontWeight: 900, fontSize: 16 }}>{ev.new_status}</span>
                          </div>
                        </td>
